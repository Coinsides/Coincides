...
describe('Agent memories page human controls', () => {
  it('makes all 205 synthetic memories reachable while rendering at most 25 rows', async () => {
    stored = Array.from({ length: 205 }, (_, index) => ({
      ...stored[0], id: `synthetic-${index}`, content: `Memory number ${index}`,
    }));
    render(<MemoryRouter><AgentMemoriesPage /></MemoryRouter>);
    await screen.findByText('Memory number 0');
    const seen: string[] = [];
    const navigation = within(screen.getByRole('navigation', { name: 'Memory pages' }));
    ...
    for (let page = 0; page < 9; page += 1) {
      ...
      expect(rows.length).toBe(page === 8 ? 5 : 25);
      seen.push(...rows.map((row) => row.querySelector('p')!.textContent!));
      if (page < 8) fireEvent.click(navigation.getByRole('button', { name: 'Next' }));
    }
    expect(new Set(seen).size).toBe(205);
    expect(seen).toEqual(stored.map((memory) => memory.content));
    ...
    expect(screen.getByText('Memory number 175')).toBeTruthy();
    expect(requests.get).toHaveBeenCalledTimes(1);
  });

  it('keeps an edit on its page and returns to a valid page after deleting the only last-page row', async () => {
    stored = Array.from({ length: 26 }, (_, index) => ({
      ...stored[0], id: `synthetic-${index}`, content: `Memory number ${index}`,
    }));
    render(<MemoryRouter><AgentMemoriesPage /></MemoryRouter>);
    await screen.findByText('Memory number 0');
    ...
    expect((navigation.getByRole('button', { name: 'Previous' }) as HTMLButtonElement).disabled).toBe(true);
    fireEvent.change(screen.getByRole('textbox'), { target: { value: '  Last-page edit\nPreserve spacing  ' } });
    fireEvent.click(screen.getByRole('button', { name: 'Save' }));
    await waitFor(() => expect(screen.queryByRole('textbox')).toBeNull());
    expect(stored[25].content).toBe('  Last-page edit\nPreserve spacing  ');
    fireEvent.click(screen.getByRole('button', { name: 'Delete' }));
    expect((navigation.getByRole('button', { name: 'Previous' }) as HTMLButtonElement).disabled).toBe(true);
    fireEvent.click(screen.getByRole('button', { name: 'Confirm delete' }));
    await screen.findByText('Memory number 0');
    expect(screen.getAllByRole('listitem')).toHaveLength(25);
    expect(screen.queryByRole('navigation', { name: 'Memory pages' })).toBeNull();
  ...
  it('saves the exact inline edit and shows the persisted content when reopened', async () => {
    const page = render(<MemoryRouter><AgentMemoriesPage /></MemoryRouter>);
    const row = await firstRow();
