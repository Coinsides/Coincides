...
function ExportPreviewMedia({ row }: { row: ExportPreviewRow }) {
  if (row.block.block_type === 'component') return <ComponentBlockProjection block={row.block} print />;
  if (row.block.block_type === 'table') return <TableBlockProjection block={row.block} print />;
      ...
      <div className={styles.exportPreviewList}>
        {rows.length === 0 ? (
          ...
          <div key={`${label}-${row.block.id}`} className={styles.exportPreviewRow}
            style={row.block.block_type === 'media' || row.block.block_type === 'table' || row.block.block_type === 'component' ? { minWidth: 0 } : undefined}>
