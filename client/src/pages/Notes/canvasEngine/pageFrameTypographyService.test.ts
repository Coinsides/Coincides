...
describe('page-frame typography families and physical baseline', () => {
  ...
  it.each([
  ...
  ])('$pageSize keeps full-precision mapping and quantizes derived 10pt metrics only', ({ pageSize, physicalWidthMm }) => {
    ...
    const exactFontSize = (10 * 96 / 72) / scale;
    const exactMetrics = {
    ...
    expect(Math.abs(profile.fontSizePx * scale * 72 / 96 - 10) / 10).toBeLessThanOrEqual(0.005);
  ...
  it('reads an existing frame width for mapping without replacing its geometry', () => {
    ...
    const profile = createPageFrameDefaultTypographyProfile(frame);
    expect(Math.abs(profile.fontSizePx - (10 * 96 / 72) / mapping.physicalScale)).toBeLessThanOrEqual(0.05);
    expect(frame).toEqual(geometryBefore);
  });
