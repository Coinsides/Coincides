...
export function addNoteCoverPage(collection: PageFrameCollectionModel, frameId: string): PageFrameCollectionModel {
  ...
  const paper = inheritNotebookPaperGeometry(collection, source);
  const frame: PageFrameModel = { ...paper, id: frameId, role: 'secondary_page_frame',
    y: Math.min(...collection.pageFrames.map((page) => page.y)) - paper.height - 80,
