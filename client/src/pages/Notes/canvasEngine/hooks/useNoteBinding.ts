...
export function useNoteBinding(noteId: string | undefined, persist: (value: NoteBindingSettings, collection?: PageFrameCollectionModel) => Promise<NoteBindingSettings | null | void>) {
  ...
  const save = useCallback(async (value: NoteBindingSettings, collection?: PageFrameCollectionModel) => {
    const request = generation.current;
    const sequence = ++saveSequence.current;
    const saved = collection ? await persist(value, collection) : await persist(value);
    if (request === generation.current && sequence === saveSequence.current) setState({
      noteId, value: saved === undefined ? value : saved, loading: false, error: null,
    });
  }, [persist, noteId]);
