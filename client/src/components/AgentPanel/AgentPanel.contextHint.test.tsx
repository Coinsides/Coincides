...
describe('B2 visible, passive context on the real panel and message transport', () => {
  it('C2 carries selected block identities once, ignores retired card metadata, and keeps the answer in the conversation', async () => {
    ...
    expect(response).toMatchObject({ id: 'durable-answer', content: 'An anchored answer' });
    expect(response.meta).toBeUndefined();
    expect(screen.getByText('An anchored answer')).toBeTruthy();
    expect(screen.queryByRole('complementary', { name: '页边答卡' })).toBeNull();
    expect(screen.queryByRole('button', { name: '插入为块' })).toBeNull();
    // Old rows remain readable as history without creating a new projection.
    const historical = { ...response, meta };
    http.get.mockResolvedValueOnce({ data: [historical] });
    await act(async () => useAgentStore.getState().fetchMessages('conversation-a'));
    expect(useAgentStore.getState().messages[0]).toEqual(historical);
    expect(screen.getByText('An anchored answer')).toBeTruthy();
    expect(screen.queryByRole('complementary', { name: '页边答卡' })).toBeNull();
  });

  it.each([
    ['done', undefined], ['eof', undefined], ['done', {}], ['eof', {}],
  ])('C2 uses the authoritative final body without answer metadata after tool-round narration (%s, %j)', async (ending, meta) => {
    ...
    const messages = useAgentStore.getState().messages;
    const response = messages[messages.length - 1];
    expect(response).toMatchObject({ id: 'durable-final', content: 'Final explanation.' });
    expect(response.meta).toBeUndefined();
  ...
  it('retains the live intent plan in message metadata without restoring the retired card projection', async () => {
    const intentPlan = { kind: 'note_patch', rule: 'replace-text', status: 'pending', steps: [] };
    const meta = { intent_plan: intentPlan, answer_card: { question: 'Old card',
      selection: { note_id: 'note-a', block_ids: ['block-a'] } } };
    vi.mocked(fetch).mockResolvedValueOnce(new Response([
      `event: message_meta\ndata: ${JSON.stringify({ message_id: 'plan-message', meta, content: 'Review the plan.' })}\n\n`,
      'event: done\ndata: {}\n\n',
    ].join(''), { headers: { 'Content-Type': 'text/event-stream' } }));
    await act(async () => useAgentStore.getState().sendMessage('Replace the selected text'));
    const messages = useAgentStore.getState().messages;
    expect(messages[messages.length - 1]).toMatchObject({ id: 'plan-message', content: 'Review the plan.' });
    expect(messages[messages.length - 1].meta).toEqual({ intent_plan: intentPlan });
  });
