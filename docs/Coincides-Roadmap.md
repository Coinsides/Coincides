...
这是从“在线工具”到“知识操作系统”的分水岭。

### v2.0 核心：笔记系统 + 社区模板

**Notebook（笔记）—— 新层级**：课程下新增「笔记」层，位于 Course 与 Goal/Deck 之间。笔记 = 结构化的课堂知识文档，交织着文字段落、嵌入卡片、关联目标、图片/图表。

```
Course（课程）
  └── Notebook（笔记 = “一本书的一章”）
        ├── 文字段落（讲解、流程、步骤）
        ├── 嵌入卡片（Definition / Theorem / Formula）
        ├── 关联目标（学习里程碑）
        ├── 图片/图表（从 PDF 裁切、Desmos 、手绘图）
        └── Proof 块（可折叠、AI 可补全）
```

**笔记生成流程**：
1. 学生上传手写笔记（扫描件 / iPad 导出 PDF）
2. Claude Vision 分析每页：识别手写文字 → LaTeX，标记图片区域，识别粘贴的印刷图片
3. AI 生成笔记草稿（结构化 Definition / Theorem / Proof / Example 块）
4. 学生审阅修改 → 确认

**大文档处理架构（200页+）—— 三层 Chunking**：
- 第一层：结构发现（Haiku 轻量模型，每页只提取标题级信息 → 目录骨架）
- 第二层：分块精读（按知识单元切分，每块 10-15 页独立并行处理）
- 第三层：全局整合（用骨架 + 结构化产出，不再读原文）

**社区模板市场**：
- 笔记模板（包含 AI 行为规则：怎么识别、怎么分块、怎么补全）
- 卡片模板（自定义字段结构 + 渲染方式）
- 课堂协作（以课程为单位的学生群体共创）
- 用户投票 + 奖励机制

**前端笔记编辑器技术选型**：
- 富文本主体：Tiptap 或 BlockNote（自定义块：卡片块、目标块、Desmos 块、图片块）
- 自由画布模式（可选）：tldraw 或 Excalidraw
- 数学公式：KaTeX（已有）
- 2D 图表：Desmos API（免费嵌入，支持交互式图形）
- 3D 可视化：Desmos 3D API + Plotly.js（三维曲面、向量场、参数曲面）
...
- **高级 Proof 补全**：AI 检测证明缺失步骤，标记并建议补全（用户确认）
- **多模态笔记输入**：语音转录 + 实时课堂录音 → 笔记
