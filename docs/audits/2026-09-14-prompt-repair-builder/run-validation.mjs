// One-off evidence runner for the 2026-09-14 prompt repair order.
// Does not change the project's verification machinery or omit existing tests.
// Usage: node docs/audits/2026-09-14-prompt-repair-builder/run-validation.mjs
//   targeted | gates | server-full | server-full-serial | client-full | typechecks | closeout
...
const modes = new Set(['targeted', 'gates', 'server-full', 'server-full-serial', 'client-full', 'typechecks', 'closeout']);
if (!modes.has(mode) || (mode !== 'targeted' && requestedTests.length)) {
  throw new Error('Use targeted [server test paths], gates, server-full, server-full-serial, client-full, typechecks, or closeout.');
}
...
let targeted = discovered.filter((path) => /(?:prompt[^/\\]*|v14ContextHint)\.test\.ts$/i.test(path));
...
if (mode === 'targeted' && !targeted.length) throw new Error('No existing prompt test files found.');
...
const temporaryRoot = mkdtempSync(join(tmpdir(), 'coincides-prompt-repair-'));
...
async function execute(label, args, cwd = root) {
  ...
  console.log(`[prompt-repair] START ${label}`);
  let rawOutput = '';
  const heartbeat = setInterval(() => console.log(`[prompt-repair] ${label}: running ${Math.round((Date.now() - started) / 1000)}s`), 45_000);
  let exitCode = 1;
  let signal = null;
  try {
  ...
  saveSummary();
  console.log(`[prompt-repair] ${exitCode === 0 ? 'PASS' : 'FAIL'} ${label}; exit=${exitCode}; log=${relativeName(join(runDirectory, logName))}`);
  return exitCode;
}
...
try {
  if (mode === 'typechecks') {
    ...
    for (const pending of hqPending) console.log(`[prompt-repair] HQ PENDING ${pending.gate}: ${pending.reason}`);
  }
} catch (error) {
  ...
  saveSummary();
  console.log(`[prompt-repair] Summary: ${relativeName(summaryPath)}`);
