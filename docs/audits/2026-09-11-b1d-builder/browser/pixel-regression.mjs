...
const rows=[];
for(const preset of ['default','quiet-ink']) for(const mode of ['current','header-baseline']) {
...
const status=rows.filter(r=>r.mode==='header-baseline').every(r=>r.changedPixels===0)?'passed':'failed';
writeFileSync(new URL('pixel-regression.json',dir),JSON.stringify({status,method:'Decoded original PNG pixels, no tolerance, no resize; header-baseline keeps all current material code and rolls back only the explicit header files/title-weight.',rows},null,2));
