...
import { NotePaperHeader } from '../../../../client/src/pages/Notes/canvasEngine/layers/NotePaperHeader';
import { PaperSkinContext } from '../../../../client/src/pages/Notes/canvasEngine/PaperSkinContext';
import { buildPaperSkinStyles } from '../../../../client/src/pages/Notes/canvasEngine/paperSkinStyles';
import { buildSkinComponentStyles } from '../../../../client/src/styles/skinComponentStyles';
import { resolveSkin } from '../../../../client/src/styles/skinPresets';
...
// Current header metadata has its own two read routes; synthetic only.
api.defaults.adapter = async (config) => {
  const path = config.url;
  let data: unknown;
  if (config.method === 'get' && path === '/notes/e1-note/tags') data = { tags: [] };
  else if (config.method === 'get' && path === '/notes/e1-note/metadata') data = {
    upstream: { sources: [], notes: [], count: 0 }, downstream: { boards: [], content_groups: [], count: 0 },
  };
  else throw new Error('Unexpected E1 metadata request: ' + config.method + ' ' + path);
...
function E1BrowserFixture() {
  const inheritedPreset = (new URLSearchParams(window.location.search).get('preset') || 'default') as SkinPresetId;
  const [skinSelection, setSkinSelection] = useState<SkinSelection | null>(null);
  const skin = resolveSkin({ preset: inheritedPreset }, null, skinSelection);
  const skinStyle = { ...buildPaperSkinStyles(skin.tokens), ...buildSkinComponentStyles(skin.components), '--sk-paper-title-weight': skin.preset === 'quiet-ink' ? '400' : '700' };
  ...
  const state = { skinPreset: skin.preset, skinSelection, inheritedPreset, gear, displayScale: reading.displayScale, profile, layout, readOnly, modal, trashedCount: trashed.length, callbacks, interaction,
    open: { more: overlay.showMoreActions, layout: overlay.showLayoutPanel, trash: overlay.showBlockTrash, preview: overlay.showExportPreview, view: overlay.showViewOptions } };
  return <PaperSkinContext.Provider value={{ style: skinStyle, preset: skin.preset, selection: skinSelection, inheritedSelection: { preset: inheritedPreset },
    save: async (selection) => { record('skin', selection); setSkinSelection(selection); }, error: null, retry: () => {} }}>
    <div style={skinStyle} data-e1-skin-root={skin.preset}>
    <main style={{ padding: 36, maxWidth: 850 }}>
      <h1>B1d E1 synthetic overlay recheck</h1>
      <NotePaperHeader titleDraft="Synthetic overlay note" descriptionDraft="Current header metadata entry" contentReadOnly={readOnly}
        onTitleDraftChange={action('title')} onDescriptionDraftChange={action('description')}
        onSaveTitle={action('saveTitle')} onSaveDescription={action('saveDescription')} onHeightChange={() => {}}
        metadata={{ noteId: 'e1-note', readOnly, mode: 'page', blockCount: 3, sourceCount: 1, status: 'active', onOpenSource: action('openSource') }} />
    ...
    <div data-page-reading-control="true" className={`${styles.canvasZoomControl} ${styles.pageReadingControl}`} style={{ position: 'fixed', bottom: 24, right: 24 }}>
      <NoteChromeLayer hostMode={modal ? 'modal' : 'page'} blockTrashLoadFailed={false} blockTrashLoading={false} contentReadOnly={readOnly}
        ...
        surfaceMode="page" surfacePolicy={{ label: 'Page', nextModeLabel: 'Canvas' }}
        ...
        onToggleMoreActions={overlay.toggleMoreActions} onOpenLayoutPanel={overlay.openLayoutPanel}
...
ReactDOM.createRoot(document.getElementById('root')!).render(<HashRouter><E1BrowserFixture /></HashRouter>);
