// C2 evidence runner. Product gates remain unchanged; all model evals are scripted.
// Usage: node docs/audits/2026-09-20-c2-intent-builder/run-validation.mjs
...
if (!['gates', 'targeted', 'server-full', 'client-full', 'typecheck', 'evals'].includes(mode) || extra.length) {
  throw new Error('Use one documented C2 validation mode.');
}
...
const targeted = discovered.filter(path => /(?:Agent|Intent|NotePatch|Attention|ContextHint|Claim|Memory|TurnIdentity|Proposal|BoardSandbox|LoopRobustness|AtomicTextSave)/u.test(path)
...
const rawRoot = join(root, '.codex-tmp/c2-intent', runId);
mkdirSync(rawRoot, { recursive: true });
const temporaryParent = realpathSync(tmpdir());
const temporaryRoot = mkdtempSync(join(temporaryParent, 'coincides-c2-validation-'));
...
async function execute(label, args, cwd = root, files = [], timeoutMs = fileTimeoutMs) {
  ...
  console.log(`[c2] START ${label}`);
  const heartbeat = setInterval(() => console.log(`[c2] ${label}: ${Math.round((Date.now() - started) / 1000)}s`), 45_000);
  try {
  ...
  console.log(`[c2] ${exitCode === 0 && !timedOut ? 'PASS' : 'FAIL'} ${label}; ${step.durationMs}ms; ${step.rawLog}`);
  return step;
}
...
try {
  ...
  saveSummary();
  console.log(`[c2] Summary ${repoPath(summaryPath)}: ${summary.passed ? 'PASS' : 'FAIL'}`);
