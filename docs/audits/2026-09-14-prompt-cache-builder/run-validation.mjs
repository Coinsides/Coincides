// Evidence runner for the 2026-09-14 prompt-cache order; no gate/registry changes.
// Usage: node docs/audits/2026-09-14-prompt-cache-builder/run-validation.mjs
//   targeted | gates | server-full-serial | closeout | client-selection | client-full
...
if (!['targeted', 'gates', 'server-full-serial', 'closeout', 'client-selection', 'client-full'].includes(mode) || extra.length) {
...
const targeted = [
  ...
  join(server, 'src/__tests__/v14ContextHint.test.ts'),
];
...
const rawRoot = join(root, '.codex-tmp/2026-09-14-prompt-cache-builder');
...
const temporaryRoot = mkdtempSync(join(temporaryParent, 'coincides-prompt-cache-'));
...
async function execute(label, args, cwd = root, testFiles = []) {
  ...
  console.log(`[prompt-cache] START ${label}`);
  let output = '';
  const heartbeat = setInterval(() => {
    console.log(`[prompt-cache] ${label}: running ${Math.round((Date.now() - started) / 1000)}s`);
  }, 45_000);
  let exitCode = 1;
  let signal = null;
  try {
  ...
  saveSummary();
  console.log(`[prompt-cache] ${exitCode === 0 ? 'PASS' : 'FAIL'} ${label}; exit=${exitCode}; log=${relativeName(logPath)}`);
  return exitCode;
}
...
try {
  if (mode === 'targeted' || mode === 'server-full-serial') {
  ...
  } else if (mode === 'client-selection' || mode === 'client-full') {
    await npm(mode, ['run', 'test:unit', '--', '--maxWorkers=2',
      ...(mode === 'client-selection' ? ['src/pages/Boards/BoardPage.selection.test.tsx'] : []),
    ], client);
  } else if (mode === 'gates') {
    await sharedBuild();
    ...
    for (const pending of hqPending) console.log(`[prompt-cache] HQ PENDING ${pending.gate}`);
  ...
  saveSummary();
  console.log(`[prompt-cache] Summary: ${relativeName(evidenceSummaryPath)}`);
