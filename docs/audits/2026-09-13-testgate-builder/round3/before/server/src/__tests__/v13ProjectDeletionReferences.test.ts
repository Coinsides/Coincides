...
test('an upgraded legacy soul with a board still blocks deletion, with an actionable conflict and no partial work', async (t) => {
  const fresh = await initDb(':memory:'); t.after(() => fresh.close());
  ...
  // The historical fixture already applied every pre-057 body. Record those
  // entries before asking the real runner to apply all pending migrations.
  db.exec(`CREATE TABLE db_migrations (id TEXT PRIMARY KEY, description TEXT NOT NULL,
    applied_at TEXT NOT NULL DEFAULT (datetime('now')))`);
  const priorLedger = fresh.prepare('SELECT id, description FROM db_migrations WHERE id < ? ORDER BY id')
    .all(migration057.id) as Array<{ id: string; description: string }>;
  for (const migration of priorLedger) {
    db.prepare('INSERT INTO db_migrations (id, description) VALUES (?, ?)').run(migration.id, migration.description);
  }
