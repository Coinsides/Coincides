// Reproducible synthetic Chrome/CDP component test. Requires serve.mjs on 5192.
...
if (!presets.includes(preset)) throw new Error('Expected a B1a preset argument');
...
const port = 9342 + presets.indexOf(preset);
const profile = resolve(root, '.codex-tmp/b1a-e1-synthetic-chrome-' + preset);
...
try {
  ...
  await send('Page.navigate',{url:'http://127.0.0.1:5192/?preset=' + preset});
  ...
  await assert('Paper appearance text uses the effective skin ink', `(() => {
    ...
    const colors=[...document.querySelectorAll('[data-paper-appearance] summary,[data-paper-appearance] label > span,[data-paper-appearance] select')].map(el=>getComputedStyle(el).color);
    return {pass:colors.length===3&&colors.every(color=>color===expected),expected,colors};
  })()`);
