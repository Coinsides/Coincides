...
import type { SkinSelection, UserSettings } from '@shared/types';
...
const KEY = 'coincides-b1a-synthetic-browser-v1';
const NOW = '2026-09-11T12:00:00.000Z';
const project = { id: 'd2-project', name: 'D2 synthetic project', color: '#756a5c', status: 'active',
  description: 'Synthetic browser evidence only', user_id: 'd2-user', created_at: NOW, updated_at: NOW, skin: null as SkinSelection | null };

type StoredBlock = NoteBlock & { note_id: string; status: string };
type Fixture = {
  settings: UserSettings;
  projectSkin: SkinSelection | null;
  notes: Note[];
  blocks: StoredBlock[];
...
function seed(): Fixture {
  ...
  return { notes, blocks, collections, settings: { theme: 'dark' }, projectSkin: null };
}
...
const failures: { url: string; method: string; message: string }[] = [];
...
api.defaults.adapter = async (config) => {
  ...
  if (path === '/canvas-objects/coordinate-contract' && method === 'get') data = { coordinate_contract: 'v2' };
  ...
  else if (path === '/courses/d2-project/summary' && method === 'get') data = { course: { ...project, skin: fixture.projectSkin }, goals: [], decks: [], documents: [] };
  else if (path === '/courses/d2-project' && method === 'put') { fixture.projectSkin = payload.skin; data = { ...project, ...payload }; }
  else if (path === '/settings' && method === 'put') { fixture.settings = { ...fixture.settings, ...payload.settings }; data = fixture.settings; }
  else if (path === '/embedding/status' && method === 'get') data = { configured: false, provider_name: null, chunks: { total: 0, embedded: 0 }, memories: { total: 0, embedded: 0 } };
  else if (path === '/settings/providers' && method === 'get') data = { providers: [] };
  else if (path === '/settings/agent-memories' && method === 'get') data = [];
  else if (/^\/notes\/[^/]+\/tags$/.test(path) && method === 'get') data = { tags: [] };
  else if (/^\/notes\/[^/]+\/metadata$/.test(path) && method === 'get') data = { upstream: { sources: [], notes: [], count: 0 }, downstream: { boards: [], content_groups: [], count: 0 } };
  else if (path === `/courses/${project.id}` && method === 'get') data = { ...project, skin: null };
    ...
    if (method === 'put') {
      ...
      if ('skin' in payload) patch.metadata = { ...note.metadata, skin: payload.skin };
      if (!Object.keys(patch).length) rejectRequest(400, 'No fields to update');
  ...
  return { data: structuredClone(data), status: 200, statusText: 'OK', headers: {}, config };
    ...
    failures.push({ url: config.url || '', method: config.method || 'get', message: String(error) });
    throw error;
  }
};

useAuthStore.setState({ token: null, user: { id: 'b1a-user', name: 'B1a synthetic user', email: 'b1a@example.invalid', onboarding_completed: true, settings: fixture.settings } as any });
...
function SyntheticProject() {
  ...
  return <section style={{ padding: 32, overflow: 'auto', height: '100%' }}>
    ...
    <div style={{ display: 'flex', gap: 12, margin: '20px 0' }}>
      <button onClick={() => useUIStore.getState().openModal('course-edit', { course: { ...project, skin: fixture.projectSkin } })}>Edit project appearance</button>
    ...
    {modal?.type === 'course-edit' && <CourseModal />}
...
useUIStore.subscribe((state, previous) => {
  ...
  errorToasts.push(...state.toasts.filter((toast) => toast.type === 'error' && !previousIds.has(toast.id)).map((toast) => toast.message));
});
Object.assign(window, { __b1aSynthetic: {
  navigate: (path: string) => router.navigate(path),
  diagnostics: () => ({ requests, failures, errorToasts }),
} });
