...
const audit = resolve(dirname(fileURLToPath(import.meta.url)), 'baseline-e1');
...
const port = 9337;
const profile = resolve(root, '.codex-tmp/e1-synthetic-chrome');
...
try {
  ...
  await click('button','View info'); await shot('03-info'); await borders('Info',popover);
  await assert('Info content and More exclusion', `document.querySelector('dl').textContent.includes('Blocks3') && ${state}.open.info && !${state}.open.more`);
  await close(); await more(); await click('button','Deleted blocks'); await shot('04-trash'); await borders('Deleted blocks',popover);
