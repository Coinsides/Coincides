...
### 问题
在 CardViewModal（点击卡片查看详情）中，翻面后背面文字出现镜像。v1.3.1 的 92d0712 曾修复过 `.cardFace` 上的相同问题，但回归了。

### 根因
`CardViewModal.module.css` 的 `.modal` 容器带有 `backdrop-filter: var(--glass-blur-lg)`，这在浏览器渲染中创建新的合成层，破坏子元素的 `backface-visibility: hidden`。与 92d0712 修复的根因完全相同——任何带 `backdrop-filter` 的祖先容器都会导致 3D 翻转失效。

### 修复
- 移除 `CardViewModal.module.css` `.modal` 的 `backdrop-filter`，改用不透明背景 `var(--bg-secondary, var(--glass-surface))`
