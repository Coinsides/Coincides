...
### 用户本地环境（Windows 11）
```
路径：D:\Coinsides\First Version\Coincides\
步骤：
1. git pull
2. npm run setup（安装/更新所有依赖）
3. 创建 .env 文件填入 API Keys
4. 两个 PowerShell：
   - 后端：npx tsx server/src/index.ts
...
### 当前状态
v1.0 已发布，Round 4 全部完成。用户已在本地成功运行并测试。

### 用户已讨论的未来方向
1. **Electron 打包**：将应用打包为 .exe 安装包（electron-builder）
2. **自动更新**：electron-updater + GitHub Releases
3. **语义版本管理**：v1.x 为修复/小功能，v2.0 为大版本
4. **更新分发策略**：GitHub Releases → 用户拉取
