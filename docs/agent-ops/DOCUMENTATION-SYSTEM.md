...
### 五层之外的三类流水件(2026-08-19 补记)

下面三个目录不属于五层中的任何一层,它们是**收据与工作面**,规矩单列:

| 目录 | 性质 | 权威? | 规矩 |
|---|---|---|---|
| `docs/agent-ops/analysis/` | 分析 / 概念设计 | **默认否**(见下方例外) | 按成文时点冻结,不回改;结论要生效必须 distill 进现状层或 ADR |
| `docs/agent-ops/handoffs/` | 交接单 | 否(是收据) | 追加式;`status` 只向前翻(draft → ready → done),不回改历史内容 |
| `docs/agent-ops/claude-log/` | 行动日志 | 否(是收据) | **只追加,永不回改** —— 它是 Henry 监督 agent 工作的唯一轨迹 |

### ⭐ analysis 层的权威例外条款(2026-08-19 立)

**默认**:`analysis/` 非权威,不可作为施工依据。

**例外**:允许 `analysis/` 中出现 `权威 (Authoritative): 是` 的文件,但必须同时满足两个条件,缺一不可 ——

1. 该文件**在 `AGENT_CONTEXT.md §1 必读清单中被点名**(否则它对必读链不可见,等于不存在);
2. 该文件顶部状态头写明其**权威范围与拍板出处**(谁拍的、依据什么、可否推翻)。

**为什么留在 analysis 而不搬进 current-state**:概念设计稿承载的是*方向与理由*(为什么这么定),现状层承载的是*此刻真相*(现在是什么样)——两者生命周期不同。搬家还会打断上游引用链(会议卷、登记册、experience report 都按原路径引它)。**代价由点名制度承担**:凡权威例外件,必在必读清单可见。

**当前唯一在册的例外件**:`analysis/unified-direction-concept-design.md`(v1 权威,V12+ 与 Agent 版共同上游宪章)。
