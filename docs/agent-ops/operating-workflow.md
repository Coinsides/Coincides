...
> **纲(Henry 2026-08-21)**:Fable=上将军,Opus=将军,CLI=前锋。**上将军的职责是管理将军,不是带兵打仗。** 这是代理权的一部分含义:代理的是统帅,不是士兵。
...
## 3.5 经济学排序(Henry 2026-08-21)

**token 稀缺度:Fable 5 < Opus 5 < Codex 订阅。** 负载向下压:Fable 只做单位 token 杠杆最高的事(设计稿/plan/拍板/抽样抽检/铸版/复盘);重上下文的活(读回执、拆单、盯施工、旅程实走、复核汇总)归 Opus;执行归 Codex。Fable 不逐份读回执,靠 Opus 摘要;日志与消息从简。
