...
- **Current frontier**: **V2.BN.12「外骨骼与地板」in progress; sub-version V2.BN.12.1「编辑基座」closed 2026-08-20.** Orders 01–05 (root-cause investigation → test tooling → lifecycle/surface-authority/legacy-migration → SlashSession) landed via checkpoints `41e6d1b` / `b2a006b` / `ad301c6`. All four 08-08 symptoms root-fixed; 21 legacy ghosts migrated; unit gate 24→198 tests, wired into `verify:v2-bn8-runtime`. **12.1 旅程分数验收已完成:首轮 16/18 未通过(J9=0,存量 note 1791 字讲稿渲染为空);12.1.1 补丁修复后 J9 补验通过 ⇒ 18/18,铸版 `v2.bn.12.1.1`。** **必修① MCP 工具面进行中**:12.2a-1 第一版 FAIL(方向不成立)后重置为 1b(server 真 zod 注册表 + manifest 生成器)与 1c(机械门只吃 manifest),二者均复核 PASS 并闭环;`test:tool-face-registry` / `test:tool-face-manifest` / `check:tool-face-manifest` / `test:tool-face-parity` / `check:tool-face-parity` 五门已接入 `verify:v2-bn8-runtime`(35s→42s),**门外脚本计数归零**。Known claim boundaries: concurrent same-note re-hydration OCC not claimed (TD-6); 过渡桥回归护栏未闭合(TD-9,随 TD-7 于 12.4 退役); manifest 单 mapper 是约定非机关(TD-10); parity 门嵌套 npm 开销为运营风险候选(TD-11)。Next: **12.2a-2 收据轴**(`'mcp'/'proposed'` + 消费方不变式守卫)→ 12.2a-3 transport 骨架。
