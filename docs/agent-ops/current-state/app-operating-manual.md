...
- **提案(proposal)**:Agent 对人说话的信道。pending→人 apply/discard;**chat 里发的提案目前没有任何 UI 面可见**(黑洞,候 Staging)——材料三型(material_map/organized_note/material_reconciliation)在 CourseDetail 有完整面;
...
**不能(宪法四禁令+现状)**:写/改笔记正文(③);直建卡片(提案唯一);碰判断域(relation confirm 族);任何不可逆删除无仪式;模拟 UI(④);检索 Source Library(现状缺口);看见自己发的 chat 提案被谁接了(无面,候 Staging)。
