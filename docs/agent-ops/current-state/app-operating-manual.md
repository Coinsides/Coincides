...
- **封面/装订预设(T7,2026-09-21)**:封面必须先有封面页,之后封面语境出现「手册式」「简明式」两款版式按钮——套用只产普通 `note_ref {field}`+manual placement(题名述名⛔复制成文本,后续编辑仍写笔记真相),套后一切可继续编辑,预设是起点⛔锁定;装订面板另有「手册式装订」一键(眉中=套用时笔记题、脚中=第 N 纸、脚右可填),走现役 PUT 落库,保留 cover/coverPage。预设目录=`shared/types/notePresets.ts` 静态出厂件,零持久化表。**设计室→模板抽屉**已从占位转三款只读库存(缩略即预览,⛔抽屉内套用——套用动作只住笔记语境)。
...
**圈块问 Agent(C2)**:笔记选区「问 Agent」沿原 contextHint 的 `note_view.data.selection={note_id,block_ids[]}` 随人的消息发送,最多32块。服务端复用 read_note 投影,优先当前页、至多8页、选区内容最多16000字符,截断/遗漏显式声明。回答呈现在 Agent 会话面板,沿连续对话继续追问。页边答卡及其「散去」「插入为块」出口已于2026-09-23移除;服务端不再写 `agent_messages.meta.answer_card`,旧消息中的该键保留但不投影。圈选与选区语境、note_patch 提案的人门采纳和撤销路径保持不变。
