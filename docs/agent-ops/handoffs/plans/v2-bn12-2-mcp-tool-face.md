...
### ⭐ Spark 实测画像(**定稿 2026-08-23**,TD-8 清债单实测所得;三条均为**实证非推测**;Fable 放行 log 08-23 #5)
...
> ### ⭐ **派单判据(由本样本提炼,定稿)**
>
> **判「适不适合 Spark」的轴是「有没有裁量空间」,不是「重不重要」也不是「机不机械」。**
>
> | 单的形状 | 档位 |
> |---|---|
> | 位点可逐处点名到 `file:line` + 目标表/符号,**且禁区可穷举**(含「改动的邻居」) | ✅ Spark |
> | 需判断「改哪里」「怎么测」「哪些算同类」 | ⛔ 5.6 |
> | 含「验不了就停」这类**须承认自己做不到**的指令 | ⛔ 5.6,**除非**该指令能写成机械形式(「跑这条命令、贴输出;跑不通即停」) |
>
> **调度方此前用「重要性」犹豫过是否拿 12.2a-2 做样本 —— 那是错的轴。** 12.2a-2 需判断「哪些 `source_type` 属于本服务」,那是裁量;TD-8 五个位点是逐处亲验后钉死的行号+目标表,零裁量。
