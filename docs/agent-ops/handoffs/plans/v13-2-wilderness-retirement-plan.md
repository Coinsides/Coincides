...
## 修订四(HQ 2026-09-08):归一规则改判(详见 4b 工单补遗一)

修订三的 ε 启发式废除——归一数值由**不变量方程**直接求解(4a 双尺为唯一裁判,负 local 合法);无解行迁准备区,翻旗前 formal 面零例外。

## 收官(2026-09-08,Henry 走查②通过)

全段收官:侦察/史记 v0/准备区+分蘖/影子跑/坐标双模/执行器(四补遗)/真库迁移(checked 113·例外 12 全孤儿行入准备区)/橙盒修复/双模退役——canvas 时代落幕,纸单模,库入 v2 契约,史记前六笔在案。死代码删除归 13.6;打印复活即刻解锁(13.1 尾款)。
