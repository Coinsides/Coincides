...
3. **白空间保全元不变式(吞段可抓性的承重梁)**:`whitespace` 类合法的充要条件 = 机械可证**仅动空白**:`nonWS(canon(x)) === nonWS(x)` 为常驻元检(配自己的 killer:令 canon 吞一个字母 ⇒ 元检必红)。由此定理:**碎片丢失任何非空白字节必被 K-1 抓住**——吞段可抓性由元不变式保证,⛔ 不靠归一化函数的自觉。
