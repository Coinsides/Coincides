...
| ⭐⭐ **K-8 申报完整性(2026-08-28 复核方细一档变异发现)** | `signature_mismatch` 的**每一处产生点**都要有 fixture 打到:①claimed 后缀存在但 **MIME 不符**;②zip 魔数 + claimed 为 zip 但 MIME 不符;③zip 魔数但 claimed **非** zip;④**文本兜底**且 MIME 不符 | ⚠️ **复核方逐行单独注掉五处 `signature_mismatch`,只有一处变红(5/3),其余四处注掉后仍 8/8 全绿** ⇒ **四条可达分支无覆盖**(已逐条读码确认可达,⛔ 非死分支)。<br>⭐ **为什么值得补**:**申报缺失 = 静默降级** —— 而**「降级的证据链就是那些原拒收理由」正是本单存在的理由**。一处申报被回归悄悄丢掉,那个件会**看起来干干净净**。<br>📌 **⛔ 这不是 builder 没做到工单要求** —— K-1~K-7 它全做到了;**是工单要求的粒度不够(与 b-1 的 K-1d 同形)。** |
