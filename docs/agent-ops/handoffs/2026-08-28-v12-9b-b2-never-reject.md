...
#### ⚠️ 抽检补上了我证明里漏的**第三个前提**(Fable,2026-08-28)

我只验了**两个条件式**的包含关系,**漏了「变量在两点之间没被改写」**:
> **`mimeType` 在 `434 → 449` 之间未被改写** —— 唯一的改写点在 `445`,而它在 `detectedDefinition` 分支里,**与 `446` 的 `else if` 互斥**。

⇒ **三个前提齐了,`450` 的条件才逐字塌缩进 `434`。** ⭐ **少这一步,前两步都不成立** —— 两个条件式再像,只要中间那个变量被动过,包含关系就断了。
📌 **教训**:**证明「条件 A ⊂ 条件 B」时,⛔ 不能只比对两个表达式的字面 —— 还要证明它们读的是【同一时刻的同一个值】。**(与「坐标系混同」同族:**式子都对,活在不同的时刻里。**)

#### ⚠️ 旁注:**这个等价性是有前提的,前提可能被将来改掉**

⭐ 该证明的有效性**依赖 `445` 与 `446` 的互斥结构**。
⇒ **将来若有人把 `detectedDefinition` 分支改成非互斥**(如 fall-through),**`450` 就从等价变异变回活代码** —— **届时它必须配自己的 killer**。
⛔ **本轮不动它**;记此以免后人只看到「450 = 等价变异」这个结论,而不知道它挂在哪根柱子上。
