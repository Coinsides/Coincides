...
| ⭐⭐ **K-6 申报产生点逐点覆盖(2026-08-28 按新家法补)** | 本单声明的**三个 code,每一个都要有 fixture 打到**:①`container_entry_path_escape`(上跳条目)②`container_entry_name_decode_fallback`(非 UTF-8 条目名)③⚠️ **`container_entry_unreadable`**(条目**解压失败/CRC 坏/流中断**) | ⚠️ **拆单自查发现:原五刀覆盖了 ①(K-1/K-2/K-3)与 ②(K-4),但 ③ `container_entry_unreadable` 一刀未覆盖。**<br>⭐ **申报缺失 = 静默降级**:坏条目若被跳过而不申报,**整包会看起来干干净净**。<br>📌 **这条是「产生点逐点列举」立规当天的第一笔收益** —— ⛔ 若不列举,它会一路漏到复核细刀那一档才被抓到,**而那时已经贵了一轮**。 |
