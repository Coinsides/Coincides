...
1. **条目名解码**:**UTF-8 优先**;失败则按 zip 传统编码回退。⭐ **回退本身必须申报**(住**子件自己**的 declarations;⛔ 静默回退 = 又一次「把证据消化掉」)。
   ⚠️⚠️ **v0.5.1 订正**:**条目名解码回退 ⛔ 不是申报事项** —— 见 §6 的 K-4。上面这句「回退本身必须申报」**已作废**。
...
- **`server/package-lock.json`** —— 新依赖的**机械后果**(与 b-4 的生成件同判,裁定方已照准)
...
| ⭐⭐ **K-4 反转为【反例刀】(v0.5.1)** | 条目名为**非 UTF-8 字节**(CP437)**且内容是完美 UTF-8** ⇒ 该条目**照常成件**、**内容无损**、回退解码确定性;⭐ **而子件的 declarations 必须【不含】`non_utf8_text`** | ⚠️⚠️ **原条要求「回退必须申报」,现在正好反过来** —— 就用**暴露谎言的那个 fixture**。<br>**理由(v0.5.1,两步)**:<br>**①类型错**:子件是**内容寻址**的,而条目名编码是**出处域**事实 ⇒ 挂子件则「同内容经净名条目与 CP437 条目两路到达时,**码之有无取决于到达顺序**」⇒ **申报不可判定**。⛔ **加第七码也救不了 —— 它和假申报错在同一处,只是换了个诚实点的名字。**<br>**②无申报义务(通例两问)**:**证据会被消化吗?决策依赖它吗?** —— 条目名原始字节与 UTF-8 标志位**就躺在已存容器 blob 里**(只存不拓的红利),回退解码是**读时投影**、确定性可重推导 ⇒ **证据未消化**;且**无任何收/拒决策依赖它** ⇒ **两问皆否,无申报义务**。<br>📌 **与 jszip 案同一标准、两个答案**:那里失败集分类**【决策】依赖**被**静默改写【消化】**的条目名 —— **两问皆是**,故彼案否、此案行。<br>⛔ **不加码、⛔ 不动 `non_utf8_text` 语义**(它仍且仅指**文件内容**非 UTF-8)。<br>⭐ **名字是礼貌标签,字节是事实** —— v0.3 ④ 老法在条目名上再次适用。 |
