...
- **D-a 补注(2026-08-23,builder 停手后)**:`list_notes` **当前没有可绑定的执行器**——逻辑内联在 `routes/notes.ts:93-110` 的匿名 handler(ownership/SQL/hydrate 全嵌在 route 里),无 domain service。我点名了一个不存在的机关(与 08-21「单一 helper」同错;adjudication §7 分则)。**裁定**:在 12.2a-3 内增加前置步 **S1a 执行器提取**——把 `GET /api/notes` 的业务逻辑提取为 `server/src/services/notes.ts` 的 `listNotes({ userId, courseId?, status? })`,route 变薄壳只做 HTTP 搬运,MCP binding 调用**同一个** service。这正是「人类正门与 Agent 正门共用同一机关」的形状,不是平行机关。授权触及面追加:`server/src/routes/notes.ts`(仅该 handler hunk)+ 新建 `server/src/services/notes.ts`。killer:①route 与 binding 调用同一 `listNotes`(spy/结构契约;删任一侧的调用即红);②既有 HTTP 正控(TD-8 G-1 notes、`GET /api/notes` 相关测试)保持绿;③提取前后 REST 响应字节等价(同 fixture 快照)。不采:在 transport 重写逻辑、读 `router.stack`、loopback HTTP、placeholder binding(builder 四条否决均采)。 **C-6 口径澄清(Opus 提,采)**:C-6 禁的是新增业务语义、第二 schema、第二 handler;**不禁单一正门的零语义提取**——提取一次、两门共用,是把红线做实。另记一形:「killer 全绿但系统为空」(placeholder binding 能过等集合 killer 却不执行能力)——builder 拒绝用它换 PASS,这一拒绝本身是判据:**过 killer ≠ 有能力**,killer 须含一条真实 `tools/call` 端到端正控。
...
### 附录修订(2026-08-27,S4-2 MED-1 定):调用点必须活着

口径 (a) 补一句:**`client_call_site` 指向的函数自身必须有上层消费链(至少一跳,且链头落在真实 UI 面——组件/事件处理器)。零调用者的 repository 函数 = 死调用点,不构成人类门。**「至少一跳」是机械地板,不是可达性证明;字面与立法理由冲突时,**以理由为准**(理由:人类必须通过产品真能走到)。

先例:`get_relation` 凭死调用点(`loadRelation` 全 client 零上层调用者)进场后按本句**摘牌**——与同日 `get_content_group` 因无调用点被裁同答案同结论。死调用点函数本身记为退役候选(随收口批清)。
