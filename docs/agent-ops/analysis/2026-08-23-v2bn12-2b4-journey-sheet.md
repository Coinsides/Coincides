> **状态 (Status)**: active(**首轮实走 2026-08-23** 基线 `356f3df`,Henry 本人登录 ⇒ A 9/12 + B 6/8 未达线,根因 = 缺陷-1「applied 收据无人类入口」;**⭐ b-5 补门 + b-5-fix 后增量重走 J5/J6/B3(基线 `be71d59`)⇒ A 12/12 + B 8/8,通过线达成**,详见 §7)
...
| **J5** | 从收据 Revert | **2 篇都回来**(`active`、`trashed_at` 清空);收据转 `reverted` | ⛔ 首轮 **0**(人在产品内走不到,见 §5 缺陷-1)。**⭐ b-5 + b-5-fix 后重走 = 2**:全程 UI —— 队列 Apply → 切「已执行」→ 见 applied 行带 **Revert** → 点 Revert ⇒ 两篇回 `active`/`trashed_at` 清空,收据 `reverted`、`revert_outcome: complete`、`restored` 两条齐;页面 `Applied 1→0`、`Reverted 4→5`、该行转 **Read only**、**Revert 按钮剩 0**。详见 §7 | ~~0~~ → **2** |
| **J6** | 单条软删(n==1) | **立即执行**(不进队列)+ 落 `applied` 收据 + **该收据可撤**,撤后 note 回来 | 首轮 **1**(前两项 ✅;「可撤」⛔ 同缺陷-1)。**⭐ 重走 = 2**:n==1 立即执行、`tier=immediate`、收据 `applied`、**未进队列**;随后**从 UI 点 Revert** ⇒ note 回 `active`,收据 `reverted`/`revert_outcome: complete`;页面 `Applied 1→0`、转 Read only | ~~1~~ → **2** |
...
| **B3** | 从该收据 Revert | **全部恢复**(source-projection 样本按 **J4 同一口径** —— 即该类应为 `skipped`/`read_only_projection`,不是硬删) | ⛔ 首轮 **0**(人在产品内走不到)。**⭐ 重走 = 2**:宣告能力 → `input_required` → `confirm:true` 当场执行(`tier=immediate`、**不进队列**)→ **从 UI 点 Revert** ⇒ 两篇回 `active`,`revert_outcome: complete`、`restored` 两条齐。投影口径同 J4「未取证」 | ~~0~~ → **2** |
...
## 7. 增量重走(2026-08-23 深夜,基线 `be71d59`)

**触发**:缺陷-1 由 **b-5**(已执行收据的人类入口:Executed 视图 + Revert)与 **b-5-fix**(status 白名单闭集化 + 两处护栏)修复并各自复核 PASS。依 Fable 裁定**只重走受该缺陷影响的三步**(J5 / J6 / B3),其余步骤首轮成绩不变(J9 先例同法)。

**前置**:`:3001` 重起到新码;**阳性对照**先证新码在跑 —— `status=applied` → 200(b-5 白名单),`status=a&status=b` 与 `status[]=applied` → **400**(b-5-fix 闭集;旧码会 200)。

| # | 重走结果 | 分 |
|---|---|---|
| **J5** | 队列 Apply → 切 Executed → **applied 行带 Revert** → 点 Revert ⇒ 两篇回 `active`;收据 `reverted`/`complete`/`restored` 两条;页面 `Applied 1→0`、`Reverted 4→5`、转 Read only、**Revert 按钮剩 0** | **2** |
| **J6** | n==1 立即执行不进队列 → 从 UI Revert ⇒ note 回 `active`、收据 `reverted`/`complete` | **2** |
| **B3** | 宣告能力 → `input_required` → `confirm:true` 当场执行(不进队列)→ 从 UI Revert ⇒ 两篇回 `active`、`restored` 两条齐 | **2** |

**⇒ 合并总分:A 车道 12/12 · B 车道 8/8。通过线(A 六步全 2 且 B 四步全 2)达成。**

**收尾核对**:全部 7 条 mcp 收据均为 `reverted`,`proposed` 计数 **0** —— 与「有能力路径不进人审队列」的设计一致。

### ⚠️ 本轮的取证降级(先写明,不含糊)

**Browser 面板在重走期间未显示** ⇒ 页面不合成帧,**坐标点击打不中**(首次 Apply 点击两次均未生效,由「页面 + DB」两源一致确认是点击没命中,不是产品问题)。
⇒ 改用 **触发按钮自身 handler(`element.click()`)** 完成点击。它**走的是产品真实的 React onClick**,但**跳过了浏览器命中测试** —— 因此**本轮不构成「按钮未被遮挡/可点」的证据**。

**该缺口由第二来源补上**:b-5 复核的 R1 在**真实浏览器 + Chrome CDP + 真实鼠标点击**下走通同一条链(登录 → Tool Receipts → Apply → Executed/Applied → Revert → note 回来),并施反刀(移除 Executed 里的 applied 合并)得到端到端真红。**⇒ 命中测试有证据,只是不来自本轮。**

**所有「note 是否真的回来」「收据状态」均以只读 DB 副本(含 `-wal`/`-shm`)为第二来源确认,不看页面文案。**
