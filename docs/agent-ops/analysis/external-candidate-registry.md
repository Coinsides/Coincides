...
>
> ### ⭐ 冗余判据(Fable 2026-08-21 裁定,log 条目 6;源自血统冗余调研)
>
> **凡评估「备胎 / 冗余 / 退路」类候选,首要判据不是能力也不是价格,是:**
>
> > **它与主力共享哪些失效模式?**
>
> **备胎的价值不在便宜,在于不与主力共享失效模式。** 一个更便宜、能力更接近、甚至同门同栈的候选,若在主力失效的那一刻**同时失效**,它就不是备胎。
>
> **执行方式——逐层数「绳子」,并显式列出覆盖与不覆盖**:
>
> ```text
> 模型级   模型本身下线/特定模型报错          ← 同家不同端点即可覆盖
> 账号级   账号被锁/档位变更/权限撤销
> 工具级   CLI 版本死锁/harness 不兼容
> 配额级   共用同一道放行闸
> 区域级   网络不可达/服务区域限制
> 供应链   经手 key 与上下文的第三方组件
> ```
>
> **⚠️ 两条纪律**:
>
> 1. **「我数到的绳子」永远可能少于「实际的绳子」** —— 结论措辞须留余量（「据现有证据」不是废话）。
>    **实例**：2026-08-21 同一天内对 Spark 的归类两次反向修正 —— 先只数到「账号+CLI」判其为额度备胎（过于乐观），后数到配额闸判其「连额度备胎都不算」（过于笃定），实测后定为「取决于一条未验行为」。**两次病根相同：拿手上现有的证据面当作全部证据面。**
> 2. **不覆盖的层要显式记成「已知缺口」,不是留白** —— 留白会被后来者读成「已覆盖」。
>
> **判例（Spark，2026-08-21 分析全文见 `2026-08-21-builder-lineage-redundancy-survey.md`）**：
>
> | 层 | Spark 是否覆盖 |
> |---|---|
> | 模型级 | ✅ **覆盖**（Sol 跑 GPU / Spark 跑 Cerebras WSE-3，同家不同端点） |
> | 账号 / 工具 / 配额闸 | ❌ **不覆盖**（同账号、同 CLI、同一道闸） |
> | 区域 / 供应链 | ❌ 不覆盖（同源） |
>
> ⇒ **判为「部分覆盖的已知缺口」**。同时这解释了一条反直觉结论：**套餐内不等于更省心** —— 套餐内会用完且共用闸（失效时刻与主力重合），**API 按量不会用完只会花钱**，后者在结构上才是备胎。这条是 $300–500 预算预留的**结构理由**，比「够用」更硬。
