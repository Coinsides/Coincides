...
> **修订一(Henry 2026-09-21 拍,依视觉设计宪章第七条)**:第 2 站经济学分流——**「样子」走受限呈现语法(数据描述:几何基元+文本+token 引用+布局配方,⛔自由 SVG/HTML),仅「行为」出码走三级转正**;行为永不现场造。呈现语法系 V15+ 独立真系统,**产房开工前须先裁其立项与否,二者顺序绑定**。第 2 站原文「OpenDesign 出皮+语法闸把关」的「语法闸」由此获得实义=血统检查(宪章第四条:⛔字面值,只收 token/角色/节奏引用)。本修订⛔改动三段律与四站流程其余部分。
