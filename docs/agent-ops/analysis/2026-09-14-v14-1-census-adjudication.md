...
9. **task-card 批量政策(2026-09-14 二轮修正,原文含事实错误)**:~~原裁"跟人门现语义(INSERT OR IGNORE 跳过+计数)"~~——**跳过+计数系旧 executor 语义,人门实为单链严格型(缺卡 404/重复 409/201 回完整链接;A2 builder 停线举证,census §4.31 在案)**。修正裁定:同门=跟人门严格语义——agent 批量=循环调人门单链 service,**同事务 all-or-nothing**(任一 404/409→整批失败,错误带肇事 card_id 明细,模型可修正重试);收据=实际创建链接 ID 清单(全成才落账,天然满足机关非空 resources 约束);对旧 executor 行为=跳过→整批失败的行为变化,申报在案。
