...
- V2.BN.8.3 之后的 TextFlow 边界引用；
- 第一版用户可见的默认 block；
- slash / insert menu 和 Structure Studio 中的 role / template 分类入口。
...
V2.BN.8.5 之后，TextFlow 和 AnnotationTruth 不应被塞进传统 block taxonomy 里解释。`TextBlock`、`TextUnit`、`InlineStructure`、`TextUnitGroup`、`writing_role` 的写作层边界见：

```text
docs/contracts/TextFlow-Contract.md
```

`AnnotationTruth`、`AnnotationSet`、`ReadingInterpretation` 和 relation endpoint reserve 见：
...
本文继续负责说明 NoteBlock / placement / block shell / special object block / template compatibility；TextFlow contract 负责自然写作层，Annotation contract 负责确认后的语义标注层。
...
V2.BN.8.3 之后，`definition` 不再是默认 active user-createable primitive family。2026-06-18 之后更准确的说法是：Definition 通常是 AnnotationTruth label / ContentGroup / ContentGroup identity/status / ReadingInterpretation proposal 的组合方向，不是 TextFlow 里的固定 knowledge_role，也不是默认 block family。
...
legacy definition-like template:
  concept_name
  description
```

普通 note 编辑可以编辑 field value 和 field layout，但不应该直接编辑 field schema。schema / role / slot 编辑属于后续 Structure Studio。
...
旧 Definition-like template 的兼容字段：
...
旧 DefinitionBlock 可以继续通过兼容 renderer 显示已有测试数据，但不再作为默认写作入口或新建入口。新内容应优先进入 TextBlock，然后通过 AnnotationTruth 标记为 definition / explanation / example / user-defined label；更严肃的内容整理应进入 ContentGroup / ContentGroup identity/status 方向。
...
template_default_layout
  TemplateDefinition 或未来 Structure Studio 定义的默认字段呈现。

block_layout_override
  单个 NoteBlock 的局部视觉覆盖，例如字段框位置、宽度、显示隐藏。
```

这两层在 V2.BN.5 只作为设计方向记录，不要求做成完整数据结构。V2.BN.6 负责正式数据边界，V2.BN.8 负责 Canvas / TextFlow 地基，后续 A9 Structure Studio 再负责产品化编辑。
...
Definition 不再进入默认 active block set。用户写 definition 时第一版应先写进 TextBlock；未来通过 selection / context menu / AI proposal 创建 AnnotationTruth label，并在需要时打包为 ContentGroup、审查其 identity/status，而不是创建新的 DefinitionBlock。

V2.BN.8.4 之后，`Heading` 也不再作为默认 active independent block family。Heading 支持文档导航和视觉结构，但它应该是：

```text
TextUnit.writing_role = heading
```

而不是一个需要独立 placement / resize / canvas object identity 的 block。

`Code` 不做同样的全量退场。行内代码和 TextBlock 内代码行可以进入 TextFlow，但 multi-line / copyable / language-aware code region 仍需要 independent CodeBlock。
...
它们可以在 field schema、media contract、table contract 和 Structure Studio 稳定后，成为用户自建或系统提供的 template variant。
...
- 用户写作时必须选择的 block；
- Definition annotation 的替代品；
- 完整 Concept 系统；
- 当前 Better Notebook 默认 insert menu 的入口。

Better Notebook 第一阶段如果用户要写“某个概念的定义”，应该优先使用 `TextBlock` 自然写作；未来再把相关选区标记为 `AnnotationTruth.raw_label = definition` 或用户自己的标签，并在需要时打包为 ContentGroup，而不是使用 `concept.basic`。
...
- category membership 控制发现、slash menu 分组、insert menu 分组和未来 Structure Studio 组织方式。
...
CategoryMembership
  只决定这个 template variant 出现在 slash / insert / Structure Studio 的哪个分组里。
```
...
- `Math`：数学测试和 LaTeX-first 使用场景的入口；第一版主要承载 Formula，以后可以加入 math-specific variant。
...
- V2.BN.8 只保留 category 规则和兼容入口；A9 Structure Studio 再把 category / role / template 管理产品化。
...
### Definition (legacy / label-marker note)
...
- legacy renderer 可以显示干净的 definition 呈现；
- legacy editing mode 可以继续允许用户编辑 name 和 description；
- 新的 definition-like meaning 应通过 AnnotationTruth / ContentGroup / Petal / ContentGroup identity/status / ReadingInterpretation proposal 表达；
- local graph / AI / export 后续应优先读取 ContentRange / ContentGroup / Petal / ContentGroup identity/status，并把 AnnotationTruth 当作重要输入信号，而不是假设一定存在 `concept_name` / `description` 字段；
...
`/definition` 不再创建独立 DefinitionBlock。V2.BN.8.5 之后它最多作为 future label/content-package command 出现，指向 selection -> AnnotationTruth -> ContentGroup -> ContentGroup identity/status / ReadingInterpretation proposal 工作流。

`/heading` 不再表示创建 independent HeadingBlock。V2.BN.8.4 起，它应优先把当前 TextUnit / TextBlock 转成 `writing_role = heading`。

`/code` 保持保守：空白或多行代码区域仍可创建 independent CodeBlock；行内代码和 TextBlock 内代码行进入 TextFlow / InlineStructure 后续路径。
...
V2.BN.8
  Canvas Engine / TextFlow 地基，并保留 category membership 与 template variant 兼容边界。

A9
  Structure Studio productization、category / role / slot 管理、用户自建 template variant。
```
