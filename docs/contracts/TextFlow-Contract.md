...
## Amendment — 2026-09-19 · V14 A4 heading roles and derived chapters

Authority: [A4 order §一](../agent-ops/handoffs/2026-09-19-v14-a4-heading-chapters-order.md) and [note-page design §1.2](../agent-ops/design/note-page-design.md#12-树章节结构). This is the single expressly authorized extension of the frozen TextFlow subset. Following the appended F17 amendment precedent, the historical frozen sections above remain unchanged; this amendment supersedes only their statement that level-specific heading roles are absent.

The closed `TextUnit.writing_role` union additionally accepts `heading_1`, `heading_2` and `heading_3`. They designate levels one, two and three. The existing `heading` value remains readable as level one, without migration. A heading is an ordinary text unit in an ordinary text block, not a new block type, chapter row, container, ownership field or knowledge entity. The flow version, identities, status rules, inline fields, UTF-16 coordinates, save revision and all other stored truth remain unchanged.

On the editable content surface, completing a line-start `# `, `## ` or `### ` prefix removes the prefix and assigns the corresponding role. `/h1`, `/h2`, `/h3` promote or change the current paragraph's level; `/body` restores paragraph presentation. The existing Heading action selects level one. A heading occupies one hard text line and its own block; visual wrapping does not create another unit. Enter, including Shift+Enter, continues into a separate paragraph block. A converted multiline unit retains its first hard line as the heading and continues the remaining text as paragraphs. Pasted headings and surrounding paragraphs retain their original sequence.

The client first applies the role/text edit through the existing text-save queue, then isolates heading units using the existing unit extraction/transfer and block-order operations. Unit transfers do not simultaneously change unit text or role. Existing unit/inline identities and range-ownership transfer rules are retained. The constituent operations occupy one existing `reversibleEdit` history entry, with reverse/forward replay of the same operations; they do not acquire a new cross-block server transaction. The preceding B7/F17 failure, revision and history-restoration rules still apply.

Heading metrics derive from the existing document typography profile (default font size 15 and line height 22). Level-one retains the existing heading ratios; levels two and three use proportional font and line-height steps. Font family, font size, line height and paragraph spacing continue to follow the same profile for measured pagination and rendering; heading metrics are not independent stored typography.

Chapters and agenda are disposable read-side projections of the ordered blocks and their current heading units. Each actual heading closes preceding headings of the same or a deeper level; its parent is the nearest preceding smaller level. Missing levels create no artificial headings, and text before the first heading remains preamble. A chapter range ends before the next same-or-smaller-level heading. The derived anchor uses the existing block and unit identities. There is no persisted chapter identity, parent, membership, page association or agenda copy.

Deleting an isolated heading block dissolves its chapter and leaves the other blocks intact. Moving its heading handle expands the chapter range into established block-order and necessary placement operations: automatic flow crosses stacks through existing frame affiliation, and manually positioned blocks follow the heading through existing coordinate conversion. The operation and its placement snapshots use one entry in the existing undo stack. Numbering and folding belong to session presentation. Folding recalculates a separate reading-page projection while the complete A1 plan remains the sole source of affiliation persistence, full-content search and printing; it adds no hidden/collapsed field to TextFlow. Existing page frames are retained, including empty pages after reflow.

Cover identity, title and description remain binding-owned. A4's content-surface heading controls exclude cover residents, and cover residents are excluded by the consumer before chapter/agenda projection. The existing server cover whitelist continues to classify ordinary text by block family; this amendment does not add a cover-specific server writing rule. A stored cover text role consequently does not establish a body chapter. The chapter projection interface is available for later consumers, but this amendment wires no Agent chapter verb or tool.
