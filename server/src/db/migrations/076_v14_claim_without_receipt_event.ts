...
export default {
  id: '076_v14_claim_without_receipt_event',
  description: 'Allow claim_without_receipt observations while preserving event history',
  up(db: Database.Database): void {
    const table = db.prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'events'")
      .get() as { sql: string };
    if (table.sql.includes("'claim_without_receipt'")) return;

    // Widen only the event vocabulary, following 073/074's transactional rebuild.
    // No domain write tool, persistent table or column is introduced.
    const sequence = db.prepare("SELECT seq FROM sqlite_sequence WHERE name = 'events'")
      .get() as { seq: number } | undefined;
    db.exec(`
      CREATE TABLE events_with_claim_observation (
        ...
        verb TEXT NOT NULL CHECK (verb IN (
          ...
          'proposal_rejected', 'published', 'claim_without_receipt'
        )),
        objects TEXT NOT NULL CHECK (json_valid(objects) AND json_type(objects) = 'array'),
        summary TEXT NOT NULL,
        meta TEXT NOT NULL DEFAULT '{}' CHECK (json_valid(meta))
      );
      INSERT INTO events_with_claim_observation (seq, ts, user_id, actor_kind, channel, verb, objects, summary, meta)
      ...
      DROP TABLE events;
      ALTER TABLE events_with_claim_observation RENAME TO events;
