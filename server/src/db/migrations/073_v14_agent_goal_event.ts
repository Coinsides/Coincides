...
export default {
  id: '073_v14_agent_goal_event',
  description: 'Allow goal_created events for the Agent write door while preserving event history',
  up(db: Database.Database): void {
    const table = db.prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'events'")
      .get() as { sql: string };
    if (table.sql.includes("'goal_created'")) return;

    // Rebuild only the closed verb CHECK, preserving every historical value and
    // the AUTOINCREMENT high-water mark. The migration runner owns the transaction.
    const sequence = db.prepare("SELECT seq FROM sqlite_sequence WHERE name = 'events'")
      .get() as { seq: number } | undefined;
    db.exec(`
      CREATE TABLE events_with_goal_created (
      ...
      INSERT INTO events_with_goal_created (seq, ts, user_id, actor_kind, channel, verb, objects, summary, meta)
      ...
      DROP TABLE events;
      ALTER TABLE events_with_goal_created RENAME TO events;
