...
import { execute, rollback, encode, type ExecutionReport } from './wildernessExecutor/executor.js';

export interface Options { database: string; user: string; output: string; action: 'preview' | 'execute' | 'rollback' }
export function parseArgs(args: string[]): Options {
  ...
  if (!values.get('--db') || !values.get('--user') || !values.get('--out')) throw new Error('executor_explicit_arguments_required');
  return { database: values.get('--db')!, user: values.get('--user')!, output: values.get('--out')!, action };
}
...
export function renderExecution(report: ExecutionReport): string {
  return [
    ...
    `scope=${cell(report.scopeUserId)}；event seq=${report.eventSeq}。`,
    `三表逐 note 身份守恒=${report.conservation.every(c => c.ok)}；复验=${report.invariants.checked}；formal 例外=${report.invariants.formalExceptions}。`,
    ...
    ...report.conservation.map(c => `| ${cell(c.note)} | ${c.unit} | ${c.before} | ${c.after} | ${c.original} | ${c.tray} | ${c.ok} |`), '',
    '| placement | destination | reason |', '| --- | --- | --- |',
    ...report.changes.map(c => `| ${cell(c.id)} | ${c.destination} | ${c.reason} |`), '',
    `before census: ${report.before.censusSha256}`, `after census: ${report.after.censusSha256}`, '',
...
export function run(options: Options) {
  ...
  try {
    if (options.action === 'preview') {
      const report = readShadowReport(db, options.user, 'explicit_readonly_database');
      writeFileSync(files.json, JSON.stringify(report, null, 2) + '\n');
      writeFileSync(files.markdown, renderMarkdown(report));
      return { action: 'preview', conservation: report.shadow.conservationOk };
    }
    db.pragma('foreign_keys = ON');
    const report = options.action === 'execute' ? execute(db, options.user) : rollback(db, options.user);
    ...
    try {
      const actual = readShadowReport(reader, options.user, 'explicit_readonly_database');
      if (actual.evidence.censusSha256 !== report.after.censusSha256) throw new Error('executor_post_commit_census_changed');
    } finally { reader.close(); }
    return { action: report.action, conservation: report.conservation.every(c => c.ok),
      checked: report.invariants.checked, exceptions: report.exceptions.length, eventSeq: report.eventSeq };
  } finally { db.close(); }
}
if (process.argv[1] && pathToFileURL(path.resolve(process.argv[1])).href === import.meta.url) {
  if (process.argv.slice(2).join(' ') === '--help') {
    console.log('Usage: node --import tsx scripts/v13WildernessExecute.ts --db <explicit-path> --user <scope> --out docs/audits/<new-basename> [--execute | --rollback]');
